...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  return (
    <nav
      ...
      className="fixed bottom-0 left-0 right-0 z-40 border-t border-emerald-950/10 bg-white/95 backdrop-blur-md shadow-lg"
    >
            ...
            >
              <div
                className={`relative flex items-center justify-center rounded-xl p-1.5 transition-colors ${
                  active ? 'bg-emerald-100/90 text-emerald-950' : 'bg-transparent'
                }`}
              >
                ...
                {item.badge !== null && item.badge !== undefined && (
                  <span
                    ...
                    className="absolute -top-1.5 -right-2 flex h-5 min-w-5 items-center justify-center rounded-full bg-emerald-700 px-1.5 text-[11px] font-bold text-white shadow-xs"
                  >
                    {item.badge}
                  </span>
                )}
              ...
              {active && (
                <span className="absolute bottom-1 h-1 w-8 rounded-full bg-emerald-800" />
              )}
            </button>
          );
        })}
